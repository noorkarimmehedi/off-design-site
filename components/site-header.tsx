...
export default function SiteHeader({ left }: { left?: ReactNode }) {
  return (
    <header className="relative z-20 grid grid-cols-[1fr_auto_1fr] items-start px-4 pt-6 sm:px-8 sm:pt-10">
      <div className="font-mono text-[11px] tracking-[-0.02em] text-black/35 sm:text-[12px]">
        {left ?? "arc.labs"}
      </div>
      <TransitionLink href="/" aria-label="Arc Lab Technology — home" className="-mt-1 block">
        <img src="/arc-logo.webp" alt="Arc Lab Technology" width={600} height={217} className="h-6 w-auto sm:h-8" />
      </TransitionLink>
      <LiveClock
        timeZone="Asia/Dhaka"
        options={CLOCK_OPTIONS}
        className="justify-self-end font-mono text-[11px] tabular-nums tracking-[-0.02em] text-black/35 sm:text-[12px]"
      />
    </header>
  )
}
