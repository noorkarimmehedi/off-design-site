import type React from "react"

type ProgressiveBlurProps = {
  className?: string
  backgroundColor?: string
  position?: "top" | "bottom"
  height?: string
  blurAmount?: string
  style?: React.CSSProperties
}

// A backdrop blur that fades in toward one edge, with a matching colour wash
export const ProgressiveBlur = ({
  className = "",
  backgroundColor = "#f5f4f3",
  position = "top",
  height = "150px",
  blurAmount = "4px",
  style,
}: ProgressiveBlurProps) => {
  const isTop = position === "top"

  return (
    <div
      aria-hidden="true"
      className={`pointer-events-none absolute left-0 w-full select-none ${className}`}
      style={{
        [isTop ? "top" : "bottom"]: 0,
        height,
        background: isTop
          ? `linear-gradient(to top, transparent, ${backgroundColor})`
          : `linear-gradient(to bottom, transparent, ${backgroundColor})`,
        maskImage: isTop ? "linear-gradient(to bottom, #000 50%, transparent)" : "linear-gradient(to top, #000 50%, transparent)",
        WebkitMaskImage: isTop
          ? "linear-gradient(to bottom, #000 50%, transparent)"
          : "linear-gradient(to top, #000 50%, transparent)",
        WebkitBackdropFilter: `blur(${blurAmount})`,
        backdropFilter: `blur(${blurAmount})`,
        WebkitUserSelect: "none",
        userSelect: "none",
        ...style,
      }}
    />
  )
}
