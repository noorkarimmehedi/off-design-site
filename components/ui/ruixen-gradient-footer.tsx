...
export interface RuixenGradientFooterProps {
  ...
  blur?: number;
  /** Vertical blur in viewBox units, when it should differ from `blur`. */
  blurY?: number;
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          // Fade the band's top edge so the blur's haze never ends in a hard line
          maskImage: "linear-gradient(to top, #000 70%, transparent)",
          WebkitMaskImage: "linear-gradient(to top, #000 70%, transparent)",
        }}
      >
        <svg
        ...
        >
          <defs>
            ...
            {/* Region spans the whole drawing (not each bar's box) so a
                strong blur never gets cut off into hard vertical edges. */}
            <filter
              id={`blur-${uid}`}
              ...
              height={VBH * 3}
            >
              <feGaussianBlur stdDeviation={`${blur} ${blurY ?? blur}`} />
            </filter>
          </defs>
